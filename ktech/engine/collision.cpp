...
bool KTech::Collision::MoveObject(ID<Object>& p_obj, Point p_dir)
{
	...
	// Able to move - no blocks at the end (could be that no blocking objects were found or all blocking objects were found to be push-able)
	if (blockData.size() == 0)
	{
		// Change positions
		for (size_t i = 0; i < pushData.size(); i++)
	...
	std::vector<CollisionData>& p_exitOverlapData)
{
	...
	for (size_t o = 0; o < layer->m_objects.size(); o++) // Other objects
	{
		...
		size_t originallyBlockedColliderI = 0; // Used if not found a push-able collider
		...
		for (size_t c = 0; c < thisObj->m_colliders.size(); c++) // This object's colliders
		{
			...
			for (oc = 0; oc < otherObj->m_colliders.size(); oc++) // Other object's colliders
			{
				...
				// But, if the potential result is overlap then proceed checking no matter the known result, we want to get all the overlaps occurrences.
